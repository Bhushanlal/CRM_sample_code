...
import { addInvoiceTemplate } from '../../../../duck/invoice/invoice.action';
...
export const NewAddTemplate = props => {
    ...
    const addInvoiceTemplateData = useSelector(state => state.invoice.addInvoiceTemplateData);
    const prevAddInvoiceTemplateData = usePrevious({ addInvoiceTemplateData });
    ...
    const saveTemplate = () => {
        ...
        if (getError === false && nameErr === '' && selectTemplateErr === '' ) {
            let data = {new_template: templateType, invoice_body: JSON.stringify(props.itemData), amount_received: props.receive}
            if(templateType==='1'){
                data.name = name;
            ...
            props.loader(true)
            dispatch(addInvoiceTemplate(data))
        }
    }
    ...
    useEffect(() => {
        if (prevAddInvoiceTemplateData && prevAddInvoiceTemplateData.addInvoiceTemplateData !== addInvoiceTemplateData) {
            if (addInvoiceTemplateData && _.has(addInvoiceTemplateData, 'data') && addInvoiceTemplateData.success === true) {
                props.loader(false)
                props.closeTemplate()
                let templateArr = []
                _.map(addInvoiceTemplateData.data, (data) => {
                    templateArr.push({value:data.id, label: data.name})
                })
                props.addTemplateInList(templateArr)
            }
            if (addInvoiceTemplateData && _.has(addInvoiceTemplateData, 'message') && addInvoiceTemplateData.success === false) {
                props.loader(false)
                setAddTemplateMessage(addInvoiceTemplateData.message)
            }
        }
        
    }, [prevAddInvoiceTemplateData,addInvoiceTemplateData])// eslint-disable-line react-hooks/exhaustive-deps
